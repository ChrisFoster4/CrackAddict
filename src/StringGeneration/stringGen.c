...
void stringGen(struct stringGenArgs *args)
{
	...
	while (*(args->hashFound) == false) {
		...
		for (uint8_t i = 0; i < (args->workerThreadCount); i++) {
			...
			for (genCount = (int) (args->stringBlockSize - 1);
				...
				if (str == NULL) {
					printf
					    ("Failed to allocate memory in stringGen\n");
					exit(1);
				}
				while (rc < maxStrLen) {
				...
				if (strlen(blockStart[genCount]) == 0) {
					printf("%s%d\n",
					       "stringGen generated NULL with i=",
					       i);
