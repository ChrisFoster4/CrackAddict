...
void work(struct workerThreadArgs *args)
{
    ...
    char ** startOfBlock = &args->strArr[(*args->id)*(args->stringBlockSize)];

    while (startOfBlock[0] == NULL){
        printf("%s%d\n","Waiting for block to populate. Thread:",*(args->id));
    }
    ...
    while(*args->hashFound == false) {
        ...
        if (startOfBlock[99]==NULL){
            printf("%s%d\n","preLoop fail with threadID:",*(args->id));
            exit(1);
        }

        printf("%s%d%s\n","Thread",*args->id," received more data");
        for (int i = args->stringBlockSize - 1; i >= 0; i--)
        {
            if (startOfBlock[i]==NULL){
                printf("%s%d%s%d\n","startOfBlock is null in thread:",*(args->id), " with i:",i);
                printf("%s%s\n","startOfBlock[i-1]:",startOfBlock[i-1]);
                exit(1);
            }
            if (startOfBlock[i]=='\0'){
            ...
            {
                printf("%s%s\n", "FOUND. ", startOfBlock[i]);
                printf("%s%s\n", "Hash was:", args->hashToFind);
                *args->hashFound = true;
                break;
            }
